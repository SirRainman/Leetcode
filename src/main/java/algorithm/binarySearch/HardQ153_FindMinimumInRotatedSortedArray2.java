 ...
 */
public class HardQ153_FindMinimumInRotatedSortedArray2 {
    ...
    //          1. 假设旋转点x 在right的右侧，即x < right，right = right - 1 没问题
    //          2. 假设旋转点x 为right，即x = right，right = right - 1 会越过x
    //              1. 已知 nums[mid] = nums[right] = nums[x] <= nums[left], nums[mid] <= nums[left]
    //              2. 因为 left <= mid < right，因此 mid < right = x，即此时 mid 一定在左排序数组中，因此 nums[mid] ≥ nums[left]
