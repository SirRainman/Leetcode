...
public class A906_IntervalsGroups {
    ...
    public static int getCount() {
        ...
        // TODO:为什么用小根堆来存储所有组中的最大右端点值？？？ 使每个分组的空隙尽可能的小，优先放到右端点较小的组中
        //  每个组中的区间都是按照左端点排序的，
        ...
        PriorityQueue<Integer> heap = new PriorityQueue<>();// 用来存放每组区间中的最右边的端点

        for(int[] p : intervals) {
            if( !heap.isEmpty() && heap.peek() < p[0]) { // 如果该区间能够在所有组中找到一个可以放的组
                heap.poll(); // 先把该组最右边的端点拿出来，后面再更新
            }
            heap.offer(p[1]);
        }
