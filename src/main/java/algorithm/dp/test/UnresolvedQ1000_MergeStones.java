 ...
 * 
 */
public class UnresolvedQ1000_MergeStones {
    // TODO:
    //  这道题非常难，建议看题解
    ...
    public int mergeStones(int[] stones, int K) {
        ...
        for(int len = K; len <= n; len++) {
            for(int i = 0; i + len - 1 < n; i++) {
                ...
                int cost = left == 0 ? stones[right] : stones[right] - stones[left - 1];
                dp[left][right] = Integer.MAX_VALUE;
                for(int p = left; p < right; p += K - 1) { // TODO: 注意p的取值，步长是K-1
                    dp[left][right] = Math.min(dp[left][right], dp[left][p] + dp[p + 1][right]);
                }
                if((right - left) % (K - 1) == 0) dp[left][right] += cost; // TODO：为什么只有这样才加上cost？？？因为只有这时候才可以合并成一堆
            }
        }
